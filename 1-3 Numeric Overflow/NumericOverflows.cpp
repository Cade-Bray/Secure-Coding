...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
            ...
            (increment < 0) && (result < std::numeric_limits<T>::min() - increment))
        {
            ...
            return std::numeric_limits<T>::max(); // Indicate overflow
        }
        result += increment;
    }
...
std::optional<T> subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // constexpr is used to check if T is an unsigned type at compile time
        // This is important because unsigned types have different overflow/underflow behavior
        if constexpr (std::is_unsigned<T>::value)
        {
            if (result < decrement)
            {
                ...
                return std::nullopt; // Indicate underflow
            }
        }
        // This is tricky. If you add the decrement to the numeric limit it can wrap.
        // If you subtract the decrement from the result it will inherently underflow.
